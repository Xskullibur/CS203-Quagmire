...
const BracketMatch: React.FC<MatchProps> = ({
  match,
  onMatchComplete,
  isAdmin,
}) => {
  ...
  const [winner, setWinner] = useState<{ username: string; id: string } | null>(
    match.winner ?? null
  );
  ...
  const completeMatchRequest = async (winnerId: string) => {
    ...
    try {
      await axiosInstance.put(
        `${API_URL}/match/${match.matchId}/complete`,
        matchDTO
      );
  ...
  const completeMatch = () => {
    ...
    if (p1Score === p2Score) {
      showAlert({
        ...
        title: "Warning",
        description:
          "The match cannot end in a tie. Adjust the scores to determine a winner.",
        confirmText: "OK",
      });
      return;
    }
    ...
    showConfirm({
      variant: "confirm",
      title: "Are you sure?",
      description: "Are you sure you want to complete this match?",
  ...
  const handleForfeit = (player: "player1" | "player2") => {
    ...
    showConfirm({
      variant: "confirm",
      title: "Are you sure?",
      description: `Are you sure you want to forfeit ${player === "player1" ? player1.username : player2?.username}?`,
      confirmText: "Yes",
      cancelText: "Cancel",
      onConfirm: () => {
          ...
          <div className="text-center">
            ...
            <div className="flex items-center gap-4 mt-2">
              {isAdmin && (
                <>
                  <Button
                    onClick={() => decrementScore(setP1Score)}
                    disabled={!!winner || p1Score === 0}
                  >
                    -
                  </Button>
                  <span className="text-lg">{p1Score}</span>
                  <Button
                    onClick={() => incrementScore(setP1Score)}
                    disabled={!!winner}
                  >
                    +
                  </Button>
            ...
            <div className="text-center">
              ...
              <div className="flex items-center gap-4 mt-2">
                {isAdmin && (
                  <>
                    <Button
                      onClick={() => decrementScore(setP2Score)}
                      disabled={!!winner || p2Score === 0}
                    >
                      -
                    </Button>
                    <span className="text-lg">{p2Score}</span>
                    <Button
                      onClick={() => incrementScore(setP2Score)}
                      disabled={!!winner}
                    >
                      +
                    </Button>
