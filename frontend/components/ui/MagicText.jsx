...
const MagicText = ({ children, className = '' }) => {
    ...
    useEffect(() => {
        ...
        const animate = (star) => {
            ...
            star.offsetHeight = star.offsetHeight; // Trigger reflow
        ...
        const animateStarWithDelay = (star, delay) => {
            setTimeout(() => {
                animate(star);
                intervals.push(setInterval(animate.bind(null, star), 1000));
            }, delay);
        };
    ...
    return (
        <span ref={magicRef} className={`magic inline-block relative ${className}`}>
            {[...Array(3)].map((_, i) => (
                <span key={`star-${i}`} className="magic-star absolute block w-[clamp(20px,1.5vw,30px)] h-[clamp(20px,1.5vw,30px)]" style={{ '--star-left': '50%', '--star-top': '50%' }}>
